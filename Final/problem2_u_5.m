...
for i=2:(N-1)
  ham(i,i-1) = coeff;
  ham(i,i) = -2*coeff; % + u;  % Set interior rows
  ham(i,i+1) = coeff;
end
...
ham(N/5,N/5) = ham(N/5,N/5) + u;        %add u as a delta barrier at N/5
...
p = 1;              % momentum of particle moving towards the delta function barrier
...
amp(iplot) = max(p_plot,[],'all');   % amplitude for trasmitted

if iter>=80000      % I observed reflection after pprox 80000 for 1000 grids
amp_refl(iplot) =  max(p_plot,[],'all');
end
...
figure(4);clf;
plot(amp);  % plot last iteration 
hold on

plot(amp_refl);        % add steady state soluion to plot
xlabel('x');  ylabel('amplitude');  
title('Amplitude of reflected and transmitted wave packets');
