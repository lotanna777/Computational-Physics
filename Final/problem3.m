...
%% 
   for step=1:nstep
      ...
      if(mod(step,printstep)==0)
          ...
          sq_avm(k) = avm(k)^2;                 % square of ave
          m_sq(k) = m_sq(k)+(abs(m))^2/N/N      % ave of energy squared
          sd_fluc(k) = en_sq(k) - sq_ave(k);    % energy fluctuation
          %save the averages
      end
   end
end
...
ft = fittype( 'A*exp(-t/tau1) + B*exp(-t/tau2)','dependent',{'ave'},'independent',{'t'},...
    'coefficients',{'A','B','tau1','tau2'}) ;    

f = fit( t, ave, ft ,'StartPoint',[-2 -2 10^4 10^4])     % Start point specifies init values
