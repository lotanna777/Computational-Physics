...
% pc(N,N) = 0;                            % point charge matrix
% pc(,N/4) = 1;                          % point charge at N/4
...
phi0 = 0;     % Potential at y=L
phi = phi0 * 4/(pi*sinh(pi)) * sin(pi*x'/L)*sinh(pi*y/L); 

%* Set boundary conditions
phi(1,:) = 0;   phi(N,:) = 0;  
phi(:,1) = 1;   phi(:,N) = 0;    
fprintf('Potential at y=1 equals %g \n',phi0);
...
for iter=1:iterMax
  ...
  if( method == 1 )      %% Jacobi method %%
    for i=2:(N-1)        % Loop over interior points only
     for j=2:(N-1) 
       phi(1:N/2,(N/2+1):N) = 0;     % zero phi for arbitrary grids
       phi(1,N/4) = 1;               % point charge at (0,0.5)
       newphi(i,j) = .25*(phi(i+1,j)+phi(i-1,j)+ ...
                               phi(i,j-1)+phi(i,j+1) + pc(i,j));
    ...
    for i=2:(N-1)        % Loop over interior points only
     for j=2:(N-1)     
       ...
       newphi = 0.25*omega*(phi(i+1,j)+phi(i-1,j)+ ...
               phi(i,j-1)+phi(i,j+1) + pc(i,j))  +  (1-omega)*phi(i,j);
       changeSum = changeSum + abs(1-phi(i,j)/newphi);
       phi(i,j) = newphi;
     end
    end
